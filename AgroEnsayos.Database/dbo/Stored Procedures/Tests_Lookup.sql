CREATE PROCEDURE [dbo].[Tests_Lookup]
	@categoriaId INT
...
BEGIN
	...
	left join (select distinct pa.[ProductId] 
				from [ProductAttribute] pa 
					inner join [AttributeMappings] ae on (pa.AttributeMappingId = ae.AttributeMappingId  ) 
	...
	AND (@searchTerm = '' OR (@searchTerm != '' AND (CHARINDEX(@searchTerm, p.[Name]) > 0 
		...
		OR p.Id in (
			...
			Inner Join [AttributeMappings] am on am.AttributeMappingId = pa.AttributeMappingId
			Inner Join [AttributeCategory] ac on am.AttributeId = ac.[AttributeId]
