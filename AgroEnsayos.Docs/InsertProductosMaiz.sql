...
Select 
	...
	, CASE WHEN t.[Dias a Floracion] like '%/%' THEN 0 WHEN t.[Dias a Floracion] = 's/d' THEN 0 WHEN t.[Dias a Floracion] IN ('-', ' ') THEN NULL ELSE CAST(t.[Dias a Floracion] AS INT) END AS DiasFloracion
	, CASE WHEN t.[Dias a Madurez Fisiologica] like '%/%' THEN 0 WHEN t.[Dias a Madurez Fisiologica] = 's/d' THEN 0 WHEN t.[Dias a Madurez Fisiologica] IN ('-', ' ') THEN NULL ELSE CAST(t.[Dias a Madurez Fisiologica] AS INT) END AS DiasMadurez
	, CASE WHEN t.[Altura de Planta (cm)] like '%/%' THEN 0 WHEN t.[Altura de Planta (cm)] = 's/d' THEN 0 WHEN t.[Altura de Planta (cm)] IN ('-', ' ') THEN NULL ELSE CAST(t.[Altura de Planta (cm)] AS INT) END AS AlturaPlanta
...
FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0',
                'Excel 8.0;HDR=Yes;IMEX=1;Database=C:\Users\lucasg\Dropbox\Development\AgroEnsayos\docs\Maiz.xls', 
